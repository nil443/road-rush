...
# &nbsp;  \* `Player::checkCollisions` does sphere‑vs‑AABB checks versus `Car`s and radius checks versus `Object`s.
...
# &nbsp; 2. Independently roll pick‑up probability (`PROB\_FUEL`, `...\_SHIELD`, `...\_COIN`). Result lives on the centre line of a random free lane.

# &nbsp; 3. 60 % chance to add a `Tree`, 40 % a `House` on each roadside.

# \* Difficulty rises smoothly because the music tempo \& camera fwd‑speed share the same scalar `difficultyFactor = 1 + elapsedTime × 0.03`.
